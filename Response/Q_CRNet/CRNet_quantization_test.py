# ---------------------------------------------------------------------------------------------------------------------+
# This is the Training script for CRNet with Quantization
# Supporting reporting result in Response of "A Novel Lightwieht Complex-valued Backbone Network for CSI Feedback"
# Hzl 2022.1
# ---------------------------------------------------------------------------------------------------------------------+
import tensorflow as tf
import keras
from keras.layers import Input, Dense, BatchNormalization, Reshape, Conv2D, add, LeakyReLU, Activation, subtract, concatenate, Layer
...
def encoder_network(x):
    ...
    encoded = Dense(encoded_dim, activation='sigmoid')(x) # Note that CRNet has no quantization operation, so the linear activation function is used
